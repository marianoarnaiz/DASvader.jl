...
function gspectrogram(signal, fs; window_size=256, overlap=0.5, alpha=2.5)
...
function plot_spectrogram(spec::Spec; plot_type=:contour)
    ...
    if plot_type == :heatmap
        heatmap!(ax, spec.time, spec.freq, spec.amp', colormap=:inferno)
    elseif plot_type == :contour
        contourf!(ax, spec.time, spec.freq, spec.amp', levels=15, colormap=:inferno)
    end
